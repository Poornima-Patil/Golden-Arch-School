$(document).ready(function() {
	...
	var $fileuploader = $('input.gallery_media').fileuploader({
		...
		upload: {
			url: 'php/ajax.php?type=upload&album='+album,
			data: null,
			type: 'POST',
		...
		editor: {
			...
			onSave: function(dataURL, item) {
				...
				if (item.appended && item.data.listProps) {
					...
					$.post('php/ajax.php?type=resize&album='+album, {name: item.name, id: item.data.listProps.id, _editor: JSON.stringify(item.editor)}, function() {
						// update the image
		...
		sorter: {
			onSort: function(list, listEl, parentEl, newInputEl, inputEl) {
				...
				// send request
				$.post('php/ajax.php?type=sort&album='+album, {
		...
		afterRender: function(listEl, parentEl, newInputEl, inputEl) {
			...
			$('body').on('click', function(e) {
				...
				if ($target.is('.gallery-action-asmain') && item.data.listProps) {
					$.post('php/ajax.php?type=asmain&album='+album, {name: item.name, id: item.data.listProps.id}, function() {
		...
		onRemove: function(item) {
			// send request
			if (item.data.listProps)
				$.post('php/ajax.php?type=remove&album='+album, {
					name: item.name,
					id: item.data.listProps.id
				});
		},
	...
	// preload the files
	$.post('php/ajax.php?type=preload&album='+album, null, function(result) {
